...
class CltInfoForm(FlaskForm):
    ...
    type1 = SelectField('行业分类', choices=['化工', '钢铁', '电力', '水泥', '其他'],
                        validators=[Length(0, 64, message='长度不正确')])
    type2 = SelectField('具体类别', choices=['硫酸', '锅炉', '玻璃窑炉', '燃煤锅炉'],
                        validators=[Length(0, 64, message='长度不正确')])
    info = StringField('其他信息', id='info',validators=[Length(0, 200, message='长度不正确')])
    ...
    remark = StringField('备注',id='remark', validators=[Length(0, 200, message='长度不正确')])
